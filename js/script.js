...
// return true or false if color is light or not
function isColorLight(rgbColor, threshold = 0.5) {
  const luminance = getLuminance(rgbColor);
  return luminance > threshold;
}

// check if color is light or not
...
function printQuote() {
  ...
  // stores random color value
  const background_color = getRandomColor();

  // change text(body) and button(#loadQuote) color and border(of button) for good visibility
  const islight = isColorLight(background_color);
  if (islight) {
    document.querySelector("body").classList.remove("light");
    document.querySelector("body").classList.add("dark");
    document.getElementById("loadQuote").style.borderColor = "#000";
  } else {
    document.querySelector("body").classList.remove("dark");
    document.querySelector("body").classList.add("light");
    document.getElementById("loadQuote").style.borderColor = "#fff";
  }
